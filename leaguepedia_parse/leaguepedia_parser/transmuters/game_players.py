...
from lol_dto.classes.game import LolGame, LolGamePlayer, LolGamePlayerEndOfGameStats
...
def add_players(
    ...
    for idx, team in enumerate(game.teams):
        ...
        for player_idx, game_player in enumerate(team.players):
            ...
            game_player.endOfGameStats = LolGamePlayerEndOfGameStats(
                kills=int(player_latest_data.get('Kills', 0)),
                deaths=int(player_latest_data.get('Deaths', 0)),
                assists=int(player_latest_data.get('Assists', 0)),
                gold=int(player_latest_data.get('TotalGold', 0)),  # Campo real en ScoreboardPlayers
                cs=int(player_latest_data.get('CS', 0))  # Creep Score (minions + monsters)
            )
